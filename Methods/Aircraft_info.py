...
'''
    def aircraft_info(self, _aircraft_name_):
        
        # Retrieving the list of aircraft models from the aircraft method
        unique_aircraft_names = self.aircraft()
        
        if _aircraft_name_ not in unique_aircraft_names:
            raise ValueError(f"Aircraft '{_aircraft_name_}' not found. Please choose a valid aircraft name from the dataset. Available aircraft models:\n{unique_aircraft_names}")
        else:
            pass
        '''
            ...
            prompt_template_aircraft_specs = PromptTable(
                input_variables=['_aircraft_name_'], 
                template="Print out a table of specifications about {_aircraft_name_}?"
            )
            
            # Generate and print the specifications using the language model
            print(llm(prompt_template_aircraft_specs.format(_aircraft_name_=_aircraft_name_)))
        '''
