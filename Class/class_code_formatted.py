...
class FIIU:
    ...
    This class provides methods for downloading, processing, and analyzing flight data.
    It includes functionalities such as calculating distances, plotting flight paths, and analyzing aircraft and airport information.
    """
    ...
    def calculate_distances(self):
        """
        Calculates distances between source and destination airports in the routes DataFrame.
    
        Returns:
        dict: A dictionary containing pairs of airport IDs as keys and their corresponding haversine distances as values.
    ...
    def haversine_distance(self, lat1, lon1, lat2, lon2):
        ...
        Returns:
        float: The haversine distance between the two sets of coordinates, in kilometers.
               If any of the input values is None, a default distance of 0 is returned.
        """
    ...
    def get_coordinates(self, airport_id):
        """
        Retrieves latitude and longitude coordinates for a specified airport ID from the airports DataFrame.
    
        Parameters:
        - airport_id (int): The unique identifier of the airport for which coordinates are requested.
    
        Returns:
        tuple: A tuple containing latitude and longitude coordinates of the specified airport.
               If the airport ID is not found in the DataFrame, returns (None, None).
        """
    ...
    def plot_airports_by_country(self, country_name):
        ...
        - country_name (str): The name of the country for which airports will be plotted.
            
        Returns:
        None
            
        This method filters the airports DataFrame based on the provided country name,
        converts the data to a GeoDataFrame, and plots the airports on a world map.
        If no airports are found for the specified country, an error message is printed.
        """
    ...
    def add_distance_column(self):
        """
        Adds a 'distance' column to the routes DataFrame, representing the haversine distance
        between source and destination airports.
        
        Returns:
        None
        
        This method merges the routes DataFrame with the airports DataFrame twice to obtain
        the coordinates (latitude and longitude) for both source and destination airports.
        Then, it calculates the haversine distance between these coordinates and adds the result
    ...
    def distance_analysis(self, bins=30, show_mean=True, show_median=True):
        ...
        - show_mean (bool, optional): Whether to display a vertical line for the mean distance.
        Default is True.
        - show_median (bool, optional): Whether to display a vertical line for the median distance.
        Default is True.
        
        Returns:
        None
        
        This method filters out None values, creates a histogram of flight distances,
        and optionally adds vertical lines for the mean and median distances.
        The resulting plot provides insights into the distribution of flight distances.
        """
    ...
    def plot_flights_from_airport(self, airport_code, internal=False):
        ...
        - internal (bool, optional): Whether to filter and plot only internal flights.
        Default is False.
        
        Returns:
        None
        
        This method filters routes based on the specified airport as the source airport,
        and optionally filters internal flights. It then creates a Plotly trace for flight paths
    ...
    def plot_flights_from_country(self, country, internal=False):
        """
        Plots flight paths originating from airports in a specified country, distinguishing between
        internal and all flights. The resulting visualization is displayed on a world map.
    
        Parameters:
        - country (str): The name of the country for which flight paths will be plotted.
        - internal (bool, optional): Whether to filter and plot only internal flights. Default is False.
    
        Returns:
        None
    
        This method filters routes based on airports in the specified country, optionally filters internal flights,
        and creates a Plotly trace for flight paths. The resulting paths are displayed on a world map,
        and the title of the plot indicates whether it shows internal flights or all flights from the country.
        """
    ...
    def aircraft_info(self, _aircraft_name):
        ...
        if _aircraft_name not in self.aircrafts_data:
            raise ValueError(f"Aircraft {_aircraft_name} not found. Please choose a valid aircraft name from the dataset. "
                             f"Available aircraft models:\n{list(self.aircrafts_data.keys())}")
    ...
    def refined_plot_flights_from_country(self, country, internal=False, short_haul_cutoff=1000):
        """
        Plots flight paths originating from airports in a specified country, distinguishing between
        short-haul and long-haul flights. Additionally, provides information on total short-haul distance,
        potential CO2 emissions reduction by replacing flights with trains, and visualizes flight paths.
    
        Parameters:
        - country (str): The name of the country for which flight paths will be plotted.
        - internal (bool, optional): Whether to filter and plot only internal flights. Default is False.
        - short_haul_cutoff (float, optional): Distance threshold (in kilometers) for defining short-haul flights.
          Default is 1000.
    
        Returns:
        None
    
        This method filters routes based on airports in the specified country, calculates distances,
        and creates a Plotly trace for flight paths. It distinguishes between short-haul and long-haul flights,
        provides annotations for total short-haul distance and potential CO2 emission reductions,
