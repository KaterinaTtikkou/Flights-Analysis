...
class FIIU:
    ...
    def aircraft_info(self, _aircraft_name):
        """
        Display specifications of a given aircraft using
        the OpenAI GPT-3.5 Turbo model.

        Parameters:
        - _aircraft_name (str):
          The name of the aircraft for which specifications are requested.

        Raises:
        - ValueError:
        If the provided aircraft name is not found in the dataset.
        Instructs the user to choose a valid aircraft name
        ...
        Note:
        - Ensure that OpenAI API key is set in 'OPENAI_API_KEY' env. var.
        ...
        if not api_key:
            raise ValueError(
                "OpenAI API key not found, set the 'OPENAI_API_KEY' env. var. with your API key."
            )
        else:
    ...
    def airports(self):
        """
        Retrieve list of unique airport based on
        the 'IATA' codes in airports DataFrame.
    ...
    def airport_info(self, _airport_name):
        """
        Display specifications of given airport
        using OpenAI GPT-3.5 Turbo model.

        Parameters:
        - _airport_name (str):
        Name of airport for which specifications are requested.

        Raises:
        - ValueError: If the OpenAI API key is not found in the env. variable.
                      Instructs user to set 'OPENAI_API_KEY' env. var. with their API key.
        ...
        if not api_key:
            raise ValueError(
                "OpenAI API key not found, set the env. var. with your API key."
            )
        else:
            # Initialize the OpenAI language model
            llm = ChatOpenAI(api_key=api_key, temperature=0.9)
